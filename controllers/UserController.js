...
module.exports = class UserController {
    ...
    static async register(req, res) {
        try {
            ...
            const registrationChecked = await UserController.checkRegistration(registration);
            if (registrationChecked) {
                return res.status(409).json({ message: `Matrícula ${registration} já utilizada pelo usuário ${registrationChecked}.` });
            }
    ...
    static async login(req, res) {
        ...
        const passwordChecked = await UserController.checkPassword(password, user.password);
    ...
    static async checkUser(req, res) {
        ...
        if (req.headers.authorization) {
            try {
                ...
                currentUser = await User.findById(userDecoded.id).select("-password -__v");
